...
void QWidgetExtended::signalEvent(QMouseEvent *event, int code) {
  ...
  if (widget != NULL) {
    ...
    // I don't use explicitly the string because if one day someone changes the
    // name of the class, the compiler will output an error
...
/* This method needs to be overrided to allow CSS styling also for
 * QWidgetExtended */
