...
std::string DateUtil::get_literal_month(int m) {
  if ((m < 1) || (m > 12))
    return std::string("");
  else
    return months[m - 1];
...
std::string DateUtil::numeric2literal_day_of_week(int d) {
  if ((d < 1) || (d > 7))
    return std::string("");
  else
...
int DateUtil::literal2numeric_day_of_week(const std::string &d) {
  int i = 0;
  for (i = 0; i < 7; i++) {
    if (d == week_days[i])
      return i + 1;
...
// Attention: each month has a different number of days (e.g. 31 doesn't exist
// in february) Assume to get a valid time (i.e. no negative numbers, etc.)
Date DateUtil::increase_month(Date date) {
  Date last_day_curr_month = get_last_day_of_month(date);
  Date first_day_next_month;
  if (date.getMonth() < 12) {
    first_day_next_month = Date(1, (last_day_curr_month.getWeekDay() % 7) + 1,
                                date.getMonth() + 1, date.getYear());
  } else { // Go to next year
    first_day_next_month = Date(1, (last_day_curr_month.getWeekDay() % 7) + 1,
                                1, date.getYear() + 1);
  }
  return Date(date.getMonthDay(),
              ((date.getMonthDay() % 7) ?: 7) +
                  first_day_next_month.getWeekDay() - 1,
              first_day_next_month.getMonth(), first_day_next_month.getYear());
}
...
Date DateUtil::decrease_month(Date date) {
  ...
  if (date.getMonth() > 1) {
    last_day_prev_month =
        Date(get_days_in_month(date.getMonth() - 1, date.getYear()),
             (first_day_curr_month.getWeekDay() - 1) ?: 7, date.getMonth() - 1,
             date.getYear());
  } else { // Go to previous year
    last_day_prev_month = Date(get_days_in_month(12, date.getYear() - 1),
                               (first_day_curr_month.getWeekDay() - 1) ?: 7, 12,
                               date.getYear() - 1);
  }
  Date first_day_prev_month = get_first_day_of_month(last_day_prev_month);
  return Date(date.getMonthDay(),
              ((date.getMonthDay() % 7) ?: 7) +
                  first_day_prev_month.getWeekDay() - 1,
              last_day_prev_month.getMonth(), last_day_prev_month.getYear());
}

Date DateUtil::increase_day(Date date) {
  Date last_day_curr_month = get_last_day_of_month(date);
  if (date.getMonthDay() < last_day_curr_month.getMonthDay())
    return Date(date.getMonthDay() + 1, ((date.getWeekDay() + 1) % 7) ?: 7,
...
Date DateUtil::decrease_day(Date date) {
  if (date.getMonthDay() > 1)
    return Date(date.getMonthDay() - 1, (date.getWeekDay() - 1) ?: 7,
                date.getMonth(), date.getYear());
...
Date DateUtil::increase_year(Date date) {
  if ((date.getMonthDay() == 29) && (date.getMonth() == 2)) /* Leap year */
    return Date(1, ((date.getWeekDay() + 2) % 7) ?: 7, date.getMonth() + 1,
                date.getYear() + 1);
  else {
    int offset = 1;
    if (is_leap(date.getYear() + 1))
      offset = 2;
    return Date(date.getMonthDay(), ((date.getWeekDay() + offset) % 7) ?: 7,
