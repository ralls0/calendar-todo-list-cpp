...
void MainCalendar::display_events(Date date) {
    ...
    list<Event*> event_list = this->API->getEventByMonth(date.getMonth(), date.getYear());

    //Event *b = new Event(3, "Festune2","" ,"casaMia", new Category(3,"enigma","green"), time(0), time(0));
   /* Event *c = new Event(4, "Festune3","" ,"casaMia", new Category(3,"enigma","green"), time(0), time(0));
    Event *d = new Event(5, "Festune4","" ,"casaMia", new Category(3,"enigma","green"), time(0), time(0));
    Event *e = new Event(6, "Festune5","" ,"casaMia", new Category(3,"enigma","green"), time(0), time(0));
    Event *f = new Event(7, "Festune6","" ,"casaMia", new Category(3,"enigma","green"), time(0), time(0));*/
   ...
   /* if (this->selected_event != NULL) {
    ...
    //sovrascrive tutto xk gli eventi vanno presi mese mese non c'è controllo annoframe==annoevento && meseframe==meseevento
        for (int i = start_offset+start.getMonthDay()-1; i < (start_offset+end.getMonthDay()); i++) {
            ...
            if (this->frames[i]->children().size() == 4) {
                ...
                connect(button_show_all, &QPushButtonExtended::on_click, this, &MainCalendar::on_button_extended_click);
                this->frames[i]->layout()->addWidget(button_show_all);
                label_event->setHidden(true);
            } else if (this->frames[i]->children().size() > 4) label_event->setHidden(true);
...
void MainCalendar::remove_events_from_frame(int i) {
    ...
    //A meta-object contains information about a class that inherits QObject, e.g. class name, superclass name, properties, signals and slots.
    while (it.hasNext()) {
...
void MainCalendar::on_button_extended_click(int index) {
    ...
    label_day ->setStyleSheet("box-shadow: 0px 1px 0px 0px #fff6af; background:linear-gradient(to bottom, #ffec64 5%, #ffab23 100%);"
                               " background-color:#ffec64; border-radius:6px; border:1px solid #ffaa22; display:inline-block;cursor:pointer;"
                             " color:#333333; font-family:Arial; font-size:15px; font-weight:bold;padding:6px 24px;text-decoration:none;"
                              "text-shadow:0px 1px 0px #ffee66;  text-align: center;");
   // label_day->setText(text);
    //label_day->setStyleSheet("QLabel { width: 100%; background-color: #ffffb3; border-bottom: 1px solid #000000; margin-bottom: 2px; }");
    char stime[14];
    int z=1;
    QTableWidget *table = new QTableWidget( this->frames[index]->children().size()-2,5);
    table->setItem(0,0,new QTableWidgetItem("NAME"));
    table->setItem(0,1,new QTableWidgetItem("DESCRIPTION"));
    ...
    table->setStyleSheet(" QHeaderView::section {background-color: #646464;"
                             ...
                             "border-style: none;  border-bottom: 1px solid #fffff8;  border-right: 1px solid #fffff8;  }"
                         " QHeaderView::section:horizontal{ visibility: hidden; border-top: 1px solid #fffff8;  }"
                         "QHeaderView::section:vertical  {visibility: hidden; border-left: 1px solid #fffff8;   }");
...
QLabelEvent* MainCalendar::createLabelEvent(Event *event) {
    ...
    //connect(label_event, &QLabelEvent::keyPressed, this, &MainCalendar::on_event_key_press);
    return label_event;
}

void MainCalendar::on_button_edit_click(QPushButtonExtended *d){
    Event *e = nullptr;
    if(d!=nullptr) e = d->getEvent();
    NewEventDialog *eventDialog = new NewEventDialog(e);
    eventDialog->show();

}

void MainCalendar::on_event_click(QLabelEvent *label_event, Qt::MouseButton button) {
        Event *e = nullptr;
        if(label_event!=nullptr) e = label_event->getEvent();
        NewEventDialog *eventDialog = new NewEventDialog(e);
